...
class ServerManagerTests(TestCase):
    def setUp(self):
        ...
        self.adopted = make_server(
            slug="adopted", status=Server.Status.ADOPTED, name="Old Box"
        )
        self.featured = make_server(
            slug="feat",
            is_featured=True,
            status=Server.Status.AVAILABLE,
            name="Featured",
    ...
    def test_pending_manager_excludes_approved(self):
        ...
        approved = make_application(
            server, review_status=AdoptionApplication.ReviewStatus.APPROVED
        )
...
class ApplyViewTests(TestCase):
    ...
    def test_valid_submission_redirects_to_status_page(self):
        response = self.client.post(
            reverse("apply", args=["test-server"]), self.valid_data
        )
        application = AdoptionApplication.objects.first()
        self.assertRedirects(
            response, reverse("application_status", args=[application.pk])
        )
...
class AdminApproveActionTests(TestCase):
    ...
    def test_approve_marks_application_approved(self):
        ...
        self.assertEqual(
            self.application.review_status, AdoptionApplication.ReviewStatus.APPROVED
        )
