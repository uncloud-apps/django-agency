...
def server_detail(request: HttpRequest, slug: str) -> HttpResponse:
    server = get_object_or_404(Server, slug=slug)
    form = AdoptionApplicationForm()
    return render(
        request, "shelter/server_detail.html", {"server": server, "form": form}
    )
...
def apply(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        if request.htmx:
            return render(
                request,
                "shelter/partials/application_thanks.html",
                {
    ...
    if request.htmx:
        return render(
            request,
            "shelter/partials/application_form.html",
            {
                "form": form,
                "server": server,
            },
...
def application_status(request: HttpRequest, pk: str) -> HttpResponse:
    application = get_object_or_404(AdoptionApplication, pk=pk)
    return render(
        request, "shelter/application_status.html", {"application": application}
    )
