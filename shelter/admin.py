...
class ServerAdmin(admin.ModelAdmin):
    ...
    def portrait_preview(self, obj):
        if obj.portrait:
            ...
            return format_html('<img src="{}" style="max-height:200px">', obj.portrait.url)
        return '(no portrait)'
...
class AdoptionApplicationAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['id', 'created_at', 'reviewed_at', 'server', 'applicant_name',
                       'applicant_email', 'applicant_location',
                       'decibel_tolerance', 'why_this_server']
    actions = ['approve_selected', 'decline_selected']

    @admin.action(description='Approve selected applications')
    def approve_selected(self, request, queryset):
        for application in queryset.filter(review_status=AdoptionApplication.ReviewStatus.PENDING):
        ...
        self.message_user(request, 'Applications approved and servers marked as adopted.')
